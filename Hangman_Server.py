...
while not game_over:
    ...
    if rx_msg_str == "QUIT":
        ...
        positions = hm_game.check_and_update_letter(rx_msg_str[0])
             
        if not hm_game.win():
            msg_guess = "GUESS"
            for pos in positions:
                msg_guess += ","+str(pos) +rx_msg_str[0]
            msg_guess += "\n"
            print(msg_guess)
        ...
        game_over = True
        print(rx_msg_str[0:-7],"RECIEVED")
        if hm_game.quick_win(rx_msg_str[0:-7]):
            game_socket.send("WIN\n".encode())
            print("Player wins.")
        else:
